<template>
    <div class="w-full flex justify-center mt-4">
        <div class="max-w-2xl w-full">
            <h1 class="text-2xl font-bold mb-5 text-center">Edit Permintaan</h1>
...
const permintaanId = route.params.id;
...
onMounted(async () => {
    ...
    try {
        const response = await axios.get(`http://localhost:8000/api/permintaans/${permintaanId}`, {
            headers: {
                Authorization: `Bearer ${authStore.accessToken}`,
            }
        });
        permintaan.value = response.data.data;
    } catch (error) {
...
const submitForm = async () => {
    ...
    try {
        await axios.put(`http://localhost:8000/api/permintaans/${permintaanId}`, permintaan.value, {
            headers: {
                Authorization: `Bearer ${authStore.accessToken}`,
            }
        });
        alert('Berhasil memperbarui Permintaan!');
        router.push({ name: 'permintaanlist' });
    } catch (error) {
        console.error('There was an error updating permintaan:', error);
        alert('Gagal memperbarui data Permintaan.');
    }
};
